module github.com/mohemohe/butimili-api

go 1.16

require (
	github.com/alecthomas/template v0.0.0-20190718012654-fb15b899a751
	github.com/dgrijalva/jwt-go v3.2.0+incompatible
	github.com/globalsign/mgo v0.0.0-20181015135952-eeefdecb41b8
	github.com/go-bongo/bongo v0.10.5-0.20180517124215-761759e31d8f
	github.com/go-bongo/go-dotaccess v0.0.0-20150208020959-ae070a2d2a38 // indirect
	github.com/labstack/echo v3.3.10+incompatible // indirect
	github.com/labstack/echo/v4 v4.0.0
	github.com/mitchellh/mapstructure v1.1.2
	github.com/mohemohe/echoHelper v0.0.0-20181120071525-fbd537222685
	github.com/mohemohe/echoHelper/v4 v4.1.0 // indirect
	github.com/oleiade/reflections v1.0.0 // indirect
	github.com/sirupsen/logrus v1.2.0
	github.com/smartystreets/goconvey v1.6.4 // indirect
	github.com/swaggo/echo-swagger v1.1.0
	github.com/swaggo/swag v1.7.0
	github.com/vrischmann/envconfig v1.1.0
	golang.org/x/crypto v0.0.0-20200622213623-75b288015ac9
)
